 ...
 *
 * NOTE: this is never called directly. Only by parent 'note' reducer (defined
...
function noteList(state = {
   ...
   * then recursively return an _additional_ noteList reducer.
   *
   * Otherwise, return the actual note lists' store
   */
  if(nextAction.listArgs.length > 0) {
    /**
     * The action is asking for a nested state, like lists[workout][123ABC].
     * Let's nest it by returning an additional noteList reducer and trying again.
     */
    return {
      ...state
      , [nextAction.listArgs[0]]: noteList(state[nextAction.listArgs[0]] || {}, nextAction)
    }
  } else {
    /**
     ...
     */
    switch(action.type) {
      case Actions.INVALIDATE_NOTE_LIST: {
        return {
          ...state
          , didInvalidate: true
        }
      }
      case Actions.REQUEST_NOTE_LIST: {
        return {
          ...state
          , items: [] // array of _id's
          , isFetching: true
      ...
      case Actions.RECEIVE_NOTE_LIST: {
        if(!action.success) {
          return {
            ...state
            , items: [] // array of _id's
      ...
      case Actions.REMOVE_NOTE_FROM_LIST:
        idArray = [...state.items]
      ...
      case Actions.SET_NOTE_FILTER: {
        return {
          ...state
          , filter: action.filter
        }
      }
      case Actions.SET_NOTE_PAGINATION: {
        return {
          ...state
...
/**
 * Primary note reducer -
 *
 * This is the single source of truth for all things 'note' related within the
...
function note(state = {
  ...
  /**
   * "byId" is an object map of all note items in the store. The map's keys are
   ...
   *
   * For example, when changing the name of a note, the single note
  ...
  /**
   * "lists" corresponds to individual instances of the noteList reducer as
   * defined above.
   *
  ...
  switch(action.type) {
     ...
     */
    case Actions.REQUEST_DEFAULT_NOTE: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVE_DEFAULT_NOTE: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
    ...
    case Actions.REQUESTNOTESCHEMA: {
      nextState = {
        ...state
        , defaultItem: {
          ...state.defaultItem
    ...
    case Actions.RECEIVENOTESCHEMA: {
      if(action.success) {
        nextState = {
          ...state
          , defaultItem: {
     ...
     * SINGLE NOTE ACTIONS
     */
    case Actions.REQUEST_SINGLE_NOTE: {
      nextState = {
        ...state
        , selected: {
          id: action.id
    ...
    case Actions.RECEIVE_SINGLE_NOTE: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.ADD_SINGLE_NOTE_TO_MAP: {
      // add this note to the map
      nextState = {
        ...state
        , byId: {
          ...state.byId
    ...
    case Actions.REQUEST_CREATE_NOTE: {
      nextState = {
        ...state
        , selected: {
          id: null
    ...
    case Actions.RECEIVE_CREATE_NOTE: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_UPDATE_NOTE: {
      nextState = {
        ...state
        , selected: {
          id: action.id
    ...
    case Actions.RECEIVE_UPDATE_NOTE: {
      if(action.success) {
        nextState = {
          ...state
          , byId: {
    ...
    case Actions.REQUEST_DELETE_NOTE: {
      nextState = {
        ...state
        , selected: {
          id: action.id
    ...
    case Actions.INVALIDATE_SELECTED_NOTE: {
      nextState = {
        ...state
        , selected: {
          didInvalidate: true
